...
<script>
    ...
    function operation() {
        ...
        if (film.checked) {
            ...
            form.innerHTML = '<form action="operation.php" method="POST" enctype="multipart/form-data"><fieldset><legend>Nuovo FIlm</legend>Titolo: <input type="text" name="titolo">Genere: <input type="text" name="genere">Durata: <input type="number" name="durata" min="10" step="1"><ul><li><input type="radio" name="tariffa" value="free" id="rb1" /><label for="rb1"><img src="../img/Free.jpg" /></label></li><li><input type="radio" name="tariffa" value="premium" id="cb2" /><label for="cb2"><img src="../img/Premium.jpg" /></label></li></ul>Locandina verticale: <input type="file" name="locandinaV" accept="image/jpg">Locandina orizzontale: <input type="file" name="locandinaH" accept="image/jpg">Trama: <input type="file" name="trama" accept="txt">File film: <input type="file" name="film" accept="video/mp4 , video/webm"><button type="submit" name="op" value="1">Invia</button></fieldset></form>';
        } else if (table.checked) {
            labelTable.style.border = "1px solid green";
            labelFilm.style.border = "1px solid #fff";
            form.innerHTML = '<form action="operation.php" method="post"><fieldset><legend>Nuovo orario</legend>Data film:<input type="date" name="data" id="data">ora film:<input type="time" name="ora" id="ora">Sala:<select name="sala"><option value="1">1</option><option value="2">2</option><option value="2">3</option><option value="2">4</option></select><br><button type="submit" name="op" value="2">Aggiungi orario!</button></fieldset></form>';
        } else {
            labelFilm.style.border = "1px solid #fff";
            labelTable.style.border = "1px solid #fff";
            form.innerHTML = '';
